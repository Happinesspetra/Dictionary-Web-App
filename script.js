...
// DICTIONARY WITH MULTI-LANGUAGE SUPPORT
...
const languageSelect = document.querySelector("#language-options");

// Language-specific error messages
const errorMessages = {
  en: {
    noDefinitions: "No Definitions Found",
    sorry: "Sorry pal, we couldn't find definitions for the word you were looking for. You can try the search again at later time or head to the web instead.",
    empty: "Whoops, can't be empty..."
  },
  es: {
    noDefinitions: "No se encontraron definiciones",
    sorry: "Lo siento, no pudimos encontrar definiciones para la palabra que buscabas. Puedes intentar la búsqueda más tarde o buscar en la web.",
    empty: "¡Ups! No puede estar vacío..."
  },
  fr: {
    noDefinitions: "Aucune définition trouvée",
    sorry: "Désolé, nous n'avons pas trouvé de définitions pour le mot que vous recherchiez. Vous pouvez réessayer la recherche plus tard ou vous rendre sur le web.",
    empty: "Oups, ne peut pas être vide..."
  }
};

function updateErrorMessages(lang) {
  document.querySelector(".not-found h2").textContent = errorMessages[lang].noDefinitions;
  document.querySelector(".not-found p").textContent = errorMessages[lang].sorry;
  document.querySelector(".empty-message").textContent = errorMessages[lang].empty;
}
...
const search = async function () {
  ...
  try {
    ...
    // Update error messages based on selected language
    updateErrorMessages(language);
    
    const data = await fetch(`${apiUrl}${language}/${searchWord}`);
    ...
    for (const phonetic of wordData[0].phonetics) {
      if (phonetic.text) {
        wordPhonetics = phonetic.text;
        break;
      }
    }
    ...
    <h1>${wordSpelling}</h1>
    ${wordPhonetics ? `<p>${wordPhonetics}</p>` : ''}
    `;
    wordGrammarEl.insertAdjacentHTML("beforeend", wordGrammarHtml);

    // AUDIO - Find first phonetic with audio
    ...
    let definitionsHtml = wordMeanings
        ...
        if (definition.synonyms && definition.synonyms.length > 0) {
          synonymsHtml = `
            <div class="word-synonyms">
              <h3>${language === 'es' ? 'Sinónimos' : language === 'fr' ? 'Synonymes' : 'Synonyms'}</h3>
      ...
      <div class="meaning-item">
        ...
        <div class="meaning-content">
          <h3>${language === 'es' ? 'Significado' : language === 'fr' ? 'Signification' : 'Meaning'}</h3>
    ...
    if (wordData[0].sourceUrls && wordData[0].sourceUrls.length > 0) {
      ...
      const sourceHtml = `
      <h3>${language === 'es' ? 'Fuente' : language === 'fr' ? 'Source' : 'Source'}</h3>
      <a
        href="${meaningSource}"
        class="source-link"
        target="_blank"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (savedLanguage) {
    ...
    updateErrorMessages(savedLanguage);
  }
...
languageSelect.addEventListener("change", function() {
  updateErrorMessages(this.value);
});
